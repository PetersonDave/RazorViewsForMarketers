...
{
    public class RazorViewForMarketersFormPresenter : SitecorePresenter<RazorViewForMarketersFormModel>
    {
        protected override RazorViewForMarketersFormModel GetModel(IView view, Item dataSource)
        {
            ID id;
            ID.TryParse(view.DataSource, out id);
            var model = new RazorViewForMarketersFormModel {Form = GetForm(id), Title = Sitecore.Context.Item.Fields["Title"]};
            return model;
        }
        ...
        protected override void HandlePostBackWithModelBinding(IView view, RazorViewForMarketersFormModel model)
        {
